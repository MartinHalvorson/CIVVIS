#!/bin/zsh
# Isolated low-priority supplement to the checkpointed counterplay matrix.
# It tests whether the connected-rush signal generalizes from 4 players to a
# 6-player, science/domination-only Pangaea setting without sharing state or
# output with the primary round-robin queue.
...
if (( idle < 45.0 )); then
  printf '===== 6p_pangaea_rush_science_domination_supplemental | deferred: CPU idle %s%% | %s =====\n' \
    "$idle" "$(date '+%Y-%m-%dT%H:%M:%S%z')"
  exit 0
fi

printf '\n===== 6p_pangaea_rush_science_domination_supplemental_part1_seed31800000 | workers 4 | idle %s%% | %s =====\n' \
  "$idle" "$(date '+%Y-%m-%dT%H:%M:%S%z')"
...
nice -n 20 target/ci/ai_eval advanced_rush_connected advanced \
  ...
  --pairs 6 --jobs 4 --seed 31800000
exit_code=$?
printf '===== 6p_pangaea_rush_science_domination_supplemental_part1_seed31800000 | exit %s | %s =====\n' \

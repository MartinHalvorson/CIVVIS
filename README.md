...
[Lv 6 (Emperor)](https://www.youtube.com/watch?v=2qilc2xTGyU) is the highest level beat so far, using computer control to bridge the gap between CIVVIS and Firaxis Civ 6. I try to keep the [YouTube](https://www.youtube.com/@civvis) channel somewhat up to date with the latest progress.
...
A gene with versions (`<base>-2`, …) is judged version by version, and a family with a version "on" ships one version: its head by pooled win-rate difference when the rule turns the head on, else the best version the rule turns on.

Above the rule sit the two lists I write by hand: `tools/genes.py::OPERATOR_DEFAULT_ON` names the genes that ship **on** whatever their three columns read, and `OPERATOR_DEFAULT_OFF` names the genes that ship **off** the same way. The ledger still records what the rule alone would have answered, so each name reads as an override rather than as agreement — see `rules.operator_default_on`, `rules.operator_default_off` and `rules.batch_decisions` in `docs/gene_ledger.json` for today's lists rather than a count written here. A pin or a hold moves a default only: neither can keep a gene the rule removes from the pool, and no gene is named by both lists. Every other default changes by playing more games. Inconsistent results between tournaments is a sign to me to run more games per tournament. Right now I typically run 1667 games per tournament with 6 players/seats each, for a total of 10,002 seats.

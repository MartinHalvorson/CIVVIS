...
Above the rule sit the genes I name "on" by hand — `tools/genes.py::OPERATOR_DEFAULT_ON`, nine of them as of 2026-08-26 (`idle-faith-patronage`, `buildings-before-projects`, `deals-for-our-gain`, `founder-temple`, `lane-great-people`, `army-target-weighs-enemy`, `research-tier-premium`, `settler-screen`, `early-contact-window`). Each ships whatever its three columns read, and the ledger still records what the rule alone would have answered, so a pin reads as an override rather than as agreement. A pin moves a default only: it cannot keep a gene the rule removes from the pool, and it cannot turn a gene off. Every other default changes by playing more games. Inconsistent results between tournaments is a sign to me to run more games per tournament. Right now I typically run 1667 games per tournament with 6 players/seats each, for a total of 10,002 seats.

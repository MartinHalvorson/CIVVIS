...
# Clearing gamelock's low-level halt is not permission to start the unattended
# lane. `civvis-games on` records the separate running intent; every direct or
# recovered launch must reject a missing/stopped marker.
...
if [[ ! -r "$INTENTFILE" || "$(<"$INTENTFILE")" != running ]]; then
  print -u2 -r -- "REFUSING: verification intent is not running at $INTENTFILE; run civvis-games on first"
  exit 64
fi
...
# n=9, no wins, median 892. `civ6_play.DEFAULT_CIVVIS_VICTORY` was
# "diplomatic" for exactly these numbers; the launcher was overriding it.
#
# 2026-08-31: EXPORT REMOVED — the operator's standing goal (win a live
# science victory on King) moved the tree default to `science` (#2824), and
# this export was silently pinning every cycle back to diplomatic because the
# supervisor reads CIVVIS_VICTORY once at start. The launcher now follows
# `civ6_play.DEFAULT_CIVVIS_VICTORY`, which is the value with the evidence
# beside it. Export CIVVIS_VICTORY here only to deliberately override the tree.

...
Native match `civvis-20260922T004732Z` used source `a8c02a2468cd47a64583e127fe4e44171bc4d7d5`. It owned Aluminum at (23,32) and (21,31), both covered by Haciendas. The planner requested mines at turns 183 and 187. The host refused both requests while the Builders had movement and charges: respectively one move/two charges and two moves/four charges. The refusal events identify the existing Hacienda and our ownership. Aluminum income remained zero through turn 250, and no Bomber was observed before Korea won by Score at 251 in the continuation. Raising the Builder score would repeat a request that already failed in execution.
...
The new Lua behavior test runs the shipped actuator and its queue against a simulated host. It covers asynchronous removal, mine ordering before later Builder actions, movement exhaustion without site blacklisting, direct-build preference, refusal/throw handling, and ownership/resource/charge/visibility guards. The original source fails the replacement assertion. After integrating main `584e1fea1a57ec8668c79c485889c569c98ad09f` once, all 57 discovered Lua suites pass; 1,461 controller Python tests complete successfully with one skip. Lua 5.1 compilation, the CI shipped-type-name gate and whitespace checks pass. The initial CI run caught an uppercase status label mistaken for an improvement identifier; using a lowercase status label corrected it without changing behavior. The repaired prototype passed all CI checks, including paired cost at -0.83% per completed turn (NOISE, within +8%); final integrated checks remain enforced by shipping.

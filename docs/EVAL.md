...
## The maintenance-aware deck screens null and ships as an entrant (2026-08-18)
...
`ai_eval advanced_maintenance_deck advanced`, 6p 74×46, 9 CS, online, 250t:
...
## Sea answers: pinned, registered, and natively non-firing (2026-08-18)
...
`home_defense_objective` admits ships while matching responder domain to the
threat's tile — a galley offshore used to recruit land units that could
neither reach it nor be reached by it (`ai.rs` threat list has no domain
filter; responders were land-only).
...
**Fires-check, 20 pairs (`advanced_sea_answers advanced`, 6p 74×46, 9 CS,
online, 250t, seed 170000000): the arms are byte-identical — every metric
equal, terminal direction 0/0/20 neutral.** Natively the treatment cannot
fire: `home_defense` is a bridge treatment (off in native production), and
the barbarian-second-hull coincidence (exactly one hull, barb ship on water,
arm check live) did not occur once in 40 games. No screen was run on a
non-firing arm.

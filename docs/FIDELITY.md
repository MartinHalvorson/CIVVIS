...
2. **The buildable and purchasable menus with costs.** SHIPPED — see the
   subsection above: the menus cross per city, `can_produce` gates on them
   and the purchase pricers answer from them. Left: Corps/Army purchase
   prices, and a planner that reads the host's turn forecast.

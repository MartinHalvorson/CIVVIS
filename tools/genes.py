...
def print_versions(ledger: dict, add: str | None = None) -> int:
    ...
    with its seats, which version the operator pinned, which ships (the head),
    and — when the family is full — which version leaves before a fourth is
    added: the THIRD-best by tracked wins (operator, 2026-08-25). A family
    that is unpriced ranks by version. `--add BASE` answers for one family
    and exits 1 while it is full, so the drop happens before the add."""
    ...
    for family in families:
        base = family[0]
        head = heads.get(base, {})
        priced = [t for t in family if rows.get(t, {}).get("win_diff_pp") is not None]
        ranked = sorted(priced, key=lambda t: (rows[t]["win_diff_pp"], family.index(t)),
                        reverse=True) + [t for t in family if t not in priced]
        print(f"{base}: {len(family)} of {MAX_VERSIONS} versions · pinned "
              f"{head.get('pinned') or '—'} · ships {head.get('ships') or '—'}"
              f" · head {head.get('head') or '— (unpriced)'}")
        for place, tag in enumerate(ranked, 1):
            row = rows.get(tag, {})
            diff = row.get("win_diff_pp")
            pooled = f"{diff:+.2f} pp" if diff is not None else "unpriced"
            print(f"  {place}. v{family.index(tag) + 1} {tag:<32} tracked wins {pooled}"
                  + (f" · posterior {row['posterior_pp']:+.0f} ± {row['posterior_se_pp']:.0f} /10k"
                     f" over {row.get('posterior_screens')} screen(s)"
                     if row.get("posterior_pp") is not None else ""))
        if len(family) >= MAX_VERSIONS:

...
from civ6_control import launcher, watch  # noqa: E402
...
def play(args: argparse.Namespace) -> int:
    ...
    def record(event: dict) -> None:
        ...
        if kind == "host":
            ...
            state["seat"] = event
            state["configured"] = event.get("setup") == "ok"
    ...
    if not bootstrap_game():
        print("could not find the game window to start a game", file=sys.stderr)
        return 5
    print("clicked into a game; the agent will reconfigure and host again")

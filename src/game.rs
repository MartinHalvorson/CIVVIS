...
mod power_tests {
    ...
    fn lowland_bands_flood_then_submerge_while_barriers_restore_and_protect() {
        ...
        let barrier = Item::Building {
            building: "flood_barrier".to_string(),
        };
...
mod district_building_wonder_runtime_tests {
    ...
    fn secret_society_choice_unlocks_its_replacement_building_and_round_trips() {
        ...
        let alchemical_society = Item::Building {
            building: "alchemical_society".to_string(),
        };
        let university = Item::Building {
            building: "university".to_string(),
        };
    ...
    fn power_conversion_projects_replace_exactly_one_existing_plant() {
        ...
        let oil = Item::Project {
            project: "convert_reactor_to_oil".to_string(),
        };
...
pub struct City {
    ...
    #[serde(default)]
    pub building_eras: BTreeMap<String, usize>,
    #[serde(default)]
...
/// strategic stockpile quantities transfer immediately; Gold per turn,
/// luxury copies, and Open Borders last for the standard 30-turn agreement.
...
pub struct Player {
    ...
    #[serde(default)]
    pub strategic_resources: BTreeMap<String, f64>,
    /// Unpaid per-turn fuel demand by resource, capped at Civ VI's -20
    /// combat-strength shortage penalty.
    #[serde(default)]
    pub strategic_resource_shortages: BTreeMap<String, i32>,
...
impl Game {
    ...
    fn process_pressure(&mut self, pid: usize) {
        ...
        for cid in targets {
            ...
            for (spos, r, amt, range) in &sources {
                if *spos != cpos && self.wdist(*spos, cpos) <= *range {
                    *self
                        .cities
                        .get_mut(&cid)
                        .unwrap()
    ...
    pub fn current_great_person(
        ...
        self.rules
            .great_people
            .iter()
            .filter(|(id, spec)| spec.kind == kind && !self.retired_great_people.contains(*id))
            ...
            for (d, position) in &c.districts {
                ...
                for (kind, points) in &self.rules.districts[d.as_str()].great_person_points {
                    *earn.entry(kind.clone()).or_insert(0.0) += *points;
                    if c.wonders.contains_key("oracle") {
                        *earn.entry(kind.clone()).or_insert(0.0) += self.rules.wonders["oracle"]
                            .effects
                ...
                if d == "lavra" {
                    if self.city_has_building_family(c, "shrine") {
                        *earn.entry("writer".to_string()).or_insert(0.0) += 1.0;
                    }
                    if self.city_has_building_family(c, "temple") {
                        *earn.entry("artist".to_string()).or_insert(0.0) += 1.0;
                    }
                    ...
                    if has_worship_building {
                        *earn.entry("musician".to_string()).or_insert(0.0) += 1.0;
                    }
                }
            }
    ...
    pub fn levy_cost(&self, pid: usize, minor: usize) -> Option<f64> {
        ...
        let units: Vec<&Unit> = self
            ...
            .filter(|unit| self.rules.units[unit.kind.as_str()].class == "military")
            .collect();
        if units.is_empty() {
            return None;
        }
        let base = units
            .iter()
            .map(|unit| self.rules.units[unit.kind.as_str()].cost)
            .sum::<f64>();
    ...
    fn do_levy_military(&mut self, pid: usize, minor: usize) -> Result<(), String> {
        ...
        for unit_id in units {
            ...
            unit.levied_until = self.turn + STANDARD_DEAL_TURNS;
            unit.moves_left = 0.0;
            unit.acted = true;
        }
        Ok(())
    ...
    fn process_levies(&mut self, pid: usize) {
        ...
        for (unit_id, minor) in expired {
            let (class, current_pos) = {
                ...
                unit.moves_left = 0.0;
                unit.acted = true;
                (self.rules.units[unit.kind.as_str()].class.clone(), unit.pos)
            };
    ...
    fn process_power(&mut self, pid: usize) {
        ...
        for city_id in city_ids {
            ...
            let mut candidates: Vec<(&str, f64, f64, i32)> = fuel_specs
                .iter()
                .filter(|(_, plant, _, _, _)| {
                    self.cities.values().any(|source| {
                        if source.owner != pid
                ...
                .map(|(resource, _, power, co2, advanced)| (*resource, *power, *co2, *advanced))
                .collect();
            ...
            for (resource, power_per_unit, co2_per_power, _) in candidates {
                ...
                self.players[pid]
                    .strategic_resources
                    .insert(resource.to_string(), stock - consumed);
                *self.players[pid]
                    .power_fuel_consumed
        ...
        for position in positions {
            ...
            if let Some(city_id) = owner_city {
                if let Some(district) = district.as_deref() {
                    let removed_buildings: Vec<String> = self.cities[&city_id]
                        .buildings
                        .iter()
                        .filter(|building| {
                            self.rules.buildings[building.as_str()]
    ...
    pub fn resolve_reactor_accident(&mut self, city_id: u32, severity: u8) {
        ...
        let city = self.cities.get_mut(&city_id).unwrap();
        city.pillaged_buildings
            .insert("nuclear_power_plant".to_string());
    ...
    fn process_strategic_resources(&mut self, pid: usize) {
        ...
        let updates: Vec<(String, f64, f64)> = resources
            .iter()
            .map(|resource| {
    ...
    fn district_building_yields(&self, city: &City, building: &str) -> Yields {
        ...
        let effects = &self.rules.districts[district.as_str()].effects;
        Yields {
    ...
    fn player_tile_yields(&self, pid: usize, pos: Pos, tile: &crate::world::Tile) -> Yields {
        if tile.flooded || tile.fallout_until > self.turn {
            return Yields::default();
        }
    ...
    pub fn city_yields(&self, cid: u32) -> Yields {
        ...
        for b in &city.buildings {
            ...
            yields.culture += building
                ...
                .unwrap_or(0.0)
                * city.pop as f64;
            if self.players[city.owner].techs.contains("electricity") {
                yields.culture += building
                    .effects
        ...
        if self.dedication_active(city.owner, "free_inquiry") {
            for (district, position) in &city.districts {
                ...
                let base = self.rules.districts[district.as_str()].yields;
        ...
        if let Some(source) = self
            ...
            .find(|source| {
                source
                    .wonders
                    .contains_key("amundsen_scott_research_station")
            })
        {
    ...
    fn unit_resource_is_committed(&self, cid: u32, item: &Item) -> bool {
        ...
        let cost = spec.resource_cost * multiplier;
        cost > 0.0
            && self.cities[&cid]
    ...
    fn commit_unit_resource(&mut self, pid: usize, cid: u32, item: &Item) -> bool {
        let (unit, multiplier) = match item {
            Item::Unit { unit } => (unit, 1.0),
            Item::Formation {
                unit, formation, ..
    ...
    pub fn can_produce(&self, pid: usize, cid: u32, item: &Item) -> bool {
        ...
        match item {
            Item::Formation { unit, formation } => {
                ...
                if let Some(resource) = spec.requires_resource.as_deref() {
                    let resource_cost =
                        spec.resource_cost * if *formation == 2 { 3.0 } else { 2.0 };
            ...
            Item::Unit { unit } => {
                ...
                if let Some(res) = &spec.requires_resource {
                    if spec.resource_cost > 0.0
                        && !self.unit_resource_is_committed(cid, item)
                        && self.strategic_stockpile(pid, res) + f64::EPSILON < spec.resource_cost
                    {
                        return false;
                    }
                }
    ...
    fn pending_city_capture_actions(&self, pid: usize) -> Vec<Action> {
    ...
    pub fn legal_actions(&self, pid: usize) -> Vec<Action> {
        ...
        if !p.is_minor && !p.is_barbarian {
            ...
            for dedication in self.available_dedications(pid) {
                acts.push(Action::ChooseDedication { dedication });
            }
    ...
    fn airlift_destinations(&self, pid: usize, uid: u32) -> Vec<Pos> {
        let Some(unit) = self.units.get(&uid) else {
            return vec![];
        };
        let spec = &self.rules.units[unit.kind.as_str()];
    ...
    fn promotion_kill_rewards(&mut self, attacker: &Unit, defeated: &Unit) {
        let defeated_spec = &self.rules.units[defeated.kind.as_str()];
    ...
    fn do_contribute_project(&mut self, pid: usize, uid: u32, cid: u32) -> Result<(), String> {
    ...
    fn rock_band_purchase_cost(&self, pid: usize) -> f64 {
    ...
    fn do_buy(&mut self, pid: usize, cid: u32, unit: &str, currency: &str) -> Result<(), String> {
        ...
        let strategic_payment = self.rules.units[unit]
            .requires_resource
            .as_ref()
            .filter(|_| self.rules.units[unit].resource_cost > 0.0)
            .map(|resource| (resource.clone(), self.rules.units[unit].resource_cost));
    ...
    fn do_buy_building(
        ...
        self.cities
            ...
            .unwrap()
            .building_eras
            .insert(building.to_string(), self.world_era);
        Ok(())
    }
    ...
    fn do_propose_deal(
        ...
        if let Some(kind) = alliance {
            if !matches!(
                kind,
                "research" | "cultural" | "economic" | "military" | "religious"
            ) || !self.players[pid].civics.contains("civil_service")
                || !self.players[other].civics.contains("civil_service")
                || (kind == "research"
                    && (self.tree_effect(pid, "research_agreements") <= 0.0
        ...
        self.pending_deals.push(DiplomaticDeal {
            ...
            expires: self.turn + 10,
        });
        Ok(())
    }

    fn do_accept_deal(&mut self, pid: usize, deal_id: u32) -> Result<(), String> {
        let index = self
            .pending_deals
            .iter()
    ...
    fn do_reject_deal(&mut self, pid: usize, deal_id: u32) -> Result<(), String> {
        let index = self
            .pending_deals
            .iter()
            .position(|deal| deal.id == deal_id && deal.to == pid)
    ...
    fn resource_receive_value(&self, pid: usize, resource: &str, amount: i32) -> f64 {
        if amount <= 0 {
            return 0.0;
        }
    ...
    fn resource_give_cost(&self, pid: usize, resource: &str, amount: i32) -> f64 {
    ...
    fn open_borders_give_cost(&self, grantor: usize, receiver: usize) -> f64 {
        if self.has_open_borders(receiver, grantor) {
            return 0.0;
        }
    ...
    fn receive_items_value(&self, pid: usize, other: usize, items: &DealItems) -> f64 {
        let resources = items
            .resources
            .iter()
    ...
    fn give_items_cost(&self, pid: usize, other: usize, items: &DealItems) -> f64 {
        let resources = items
            .resources
            .iter()
    ...
    fn items_are_valid(&self, owner: usize, items: &DealItems) -> bool {
    ...
    fn transfer_gold(&mut self, payer: usize, receiver: usize, amount: f64) {
        let paid = amount.min(self.players[payer].gold).max(0.0);
        self.players[payer].gold -= paid;
        self.players[receiver].gold += paid;
    }

    fn transfer_strategic_items(&mut self, payer: usize, receiver: usize, items: &DealItems) {
        for (resource, amount) in &items.resources {
            ...
            self.players[payer]
                .strategic_resources
                .insert(resource.clone(), payer_stock - quantity);
            self.players[receiver]
                .strategic_resources
                .insert(resource.clone(), receiver_stock + quantity);
        }
    }
    ...
    /// Gather every AI's acceptable resource/Favor/Open Borders quote and
    /// sort it by benefit to the requesting player, matching Quick Deals'
    /// compare-all-counterparties workflow.
    ...
    fn process_diplomacy(&mut self, pid: usize) {
        ...
        let grievance_decay = if self.world_era <= 1 { 2.0 } else { 1.0 };
        for amount in self.players[pid].grievances.values_mut() {
            *amount = (*amount - grievance_decay).max(0.0);
        }
        self.players[pid]
            .grievances
        ...
        let occupied_founders: Vec<usize> = self
            .cities
            .values()
            .filter(|city| city.owner == pid && city.original_owner != pid)
            .map(|city| city.original_owner)
            .filter(|founder| {
                self.players
                    .get(*founder)
                    .is_some_and(|player| !player.is_barbarian)
            })
            .collect();
    ...
    pub fn available_governor_promotions(&self, pid: usize, governor: &str) -> Vec<String> {
                ...
                !state.promotions.contains(*name)
                    && (promotion.requires.is_empty()
                        || promotion
                            .requires
                            .iter()
                            .any(|required| state.promotions.contains(required)))
            })
            .map(|(name, _)| name.clone())
            .collect()
    }
    ...
    fn process_loyalty(&mut self, pid: usize) {
        ...
        for cid in self.player_city_ids(pid) {
            ...
            if self.units_at(cpos).into_iter().any(|uid| {
    ...
    pub fn available_dedications(&self, pid: usize) -> Vec<String> {
                ...
                let has_route = self.routes.iter().any(|route| {
                    route.owner == pid
                        && self
                            .cities
                            .get(&route.dest)
                            .is_some_and(|city| city.owner == rival.id)
                });
                let trade = if has_route {
                    self.policy_effect(pid, "trade_partner_tourism_pct")
                } else {
                    0.0
                };
    ...
    pub fn tourism_per_turn(&self, pid: usize) -> f64 {
        ...
        for city in self.cities.values().filter(|city| city.owner == pid) {
            ...
            for pos in &city.owned_tiles {
                ...
                if city.wonders.contains_key("golden_gate_bridge")
    ...
    fn complete_random_nodes(&mut self, pid: usize, amount: usize, technologies: bool) {
        for _ in 0..amount {
            let candidates: Vec<String> = if technologies {
                self.rules
                    .techs
                    .keys()
    ...
    fn apply_wonder_completion_effects(
        ...
        *self.players[pid]
        ...
        if self.cities[&cid].wonders.contains_key("apadana") {
            self.players[pid].envoys_free += self.rules.wonders["apadana"]
                .effects
                .get("envoys_per_wonder")
                .copied()
                .unwrap_or(0.0) as i64;
        }
        ...
        if treasury_pct != 0.0 {
            self.players[pid].gold *= 1.0 + treasury_pct / 100.0;
        }
        ...
        if effect("ancient_classical_tech_boosts") > 0.0 {
            let boosts: Vec<(String, f64)> = self
                ...
                .map(|(node, tech)| (node.clone(), tech.cost))
                .collect();
            let fraction = self.boost_frac(pid);
            for (node, cost) in boosts {
                self.players[pid].boosted_techs.insert(node.clone());
                if self.players[pid].research.as_deref() == Some(node.as_str()) {
                    self.players[pid].research_progress += fraction * cost;
                }
            }
        }
        self.complete_random_nodes(pid, effect("free_random_techs") as usize, true);
        self.complete_random_nodes(pid, effect("free_random_civics") as usize, false);
        if effect("free_atomic_civic_boosts") > 0.0 {
            let candidates: Vec<(String, f64)> = self
                ...
                .map(|(node, civic)| (node.clone(), civic.cost))
                .collect();
            if !candidates.is_empty() {
                let (node, cost) = candidates[self.rng.below(candidates.len())].clone();
                self.players[pid].boosted_civics.insert(node.clone());
                if self.players[pid].civic.as_deref() == Some(node.as_str()) {
                    self.players[pid].civic_progress += self.boost_frac(pid) * cost;
                }
            }
        }
        ...
        if free_canals > 0 {
            ...
            let mut sites: Vec<Pos> = self
                .nbrs(position)
                .into_iter()
                .filter(|site| legal.contains(site))
                .collect();
            sites.sort();
            for site in sites.into_iter().take(free_canals) {
                let tile = self.map.tiles.get_mut(&site).unwrap();
                tile.district = Some("canal".to_string());
                tile.improvement = None;
                tile.feature = None;
                tile.pillaged = false;
                self.cities
        ...
        if effect("free_foreign_continent_city_center_building") > 0.0 {
            let targets: Vec<u32> = self
                .player_city_ids(pid)
                .into_iter()
                .filter(|city_id| self.on_foreign_continent(pid, self.cities[city_id].pos))
                .collect();
            for city_id in targets {
                ...
                if let Some(building) = candidates.first() {
                    let city = self.cities.get_mut(&city_id).unwrap();
                    city.buildings.push(building.clone());
                    city.building_eras.insert(building.clone(), self.world_era);
                }
            }
        }
        if self.players[pid].dvp >= DIPLOMATIC_VICTORY_POINTS {
        ...
        match item {
            Item::Formation { unit, formation } => {
                ...
                self.cities
                    ...
                    .unwrap()
                    .strategic_resource_commitments
                    .remove(&Self::item_progress_key(item));
                self.apply_training_district_effects(cid, placed);
                let science_pct =
                    self.city_building_effect(&self.cities[&cid], "science_per_unit_cost_pct");
                self.players[pid].research_overflow += self.item_cost(item) * science_pct / 100.0;
            ...
            Item::Unit { unit } => {
                ...
                if matches!(
                    self.rules.units[unit.as_str()].class.as_str(),
                    "military" | "support"
                ) {
                    let science_pct =
                        self.city_building_effect(&self.cities[&cid], "science_per_unit_cost_pct");
                    self.players[pid].research_overflow +=
                        self.rules.units[unit.as_str()].cost * science_pct / 100.0;
                }
                if self.rules.units[unit.as_str()].domain.as_deref() == Some("sea")
                    && self.empire_wonder_effect(pid, "duplicate_naval_units") > 0.0
                {
                    ...
                    > 0.0
                {
                    ...
                    for position in lowlands {
                        ...
                        if let Some(district) = district {
                            repaired_buildings.extend(
                                self.cities[&cid]
                                    .buildings
                                    .iter()
                                    .filter(|candidate| {
                                        self.rules.buildings[candidate.as_str()]
                                            .district
                                            .as_deref()
                                            .is_some_and(|family| {
                                                self.district_is_family(&district, family)
                    ...
                    self.cities
                        ...
                        .unwrap()
                        .pillaged_buildings
                        .retain(|candidate| !repaired_buildings.contains(candidate));
                }
                true
            }
            Item::District { district, pos } => {
            ...
            Item::Wonder { wonder, pos } => {
                ...
                self.apply_wonder_completion_effects(pid, cid, wonder, *pos, &spec);
                true
            }
            Item::Repair { repair, pos } => {
                if repair == "district" {
                    ...
                    let city = self.cities.get_mut(&cid).unwrap();
                    city.pillaged_buildings.remove(repair);
                    city.building_eras.insert(repair.clone(), self.world_era);
                }
                true
            }
            Item::Project { project } => {
                ...
                if let Some(target) = Self::converted_power_plant(project) {
                    ...
                    let city = self.cities.get_mut(&cid).unwrap();
                    city.buildings
                        .retain(|building| !plants.contains(&building.as_str()));
                    city.pillaged_buildings
                        .retain(|building| !plants.contains(&building.as_str()));
                    city.building_eras
                        .retain(|building, _| !plants.contains(&building.as_str()));
                    city.buildings.push(target.to_string());
                    city.building_eras
                        .insert(target.to_string(), self.world_era);
                    city.reactor_age = 0;
                }
    ...
    fn apply_training_district_effects(&mut self, cid: u32, uid: u32) {
        let spec = self.rules.units[self.units[&uid].kind.as_str()].clone();
        if spec.domain.as_deref() == Some("sea") {
            let movement = self.city_district_effect(&self.cities[&cid], "naval_movement");
            self.units.get_mut(&uid).unwrap().bonus_moves += movement;
        }
        let city = &self.cities[&cid];
        let mut xp_pct = if matches!(
            spec.promotion_class.as_str(),
            "melee" | "anti_cavalry" | "ranged"
        ) {
            self.city_building_effect(city, "melee_anti_cavalry_ranged_xp_pct")
        } else if spec.cavalry || spec.siege {
            self.city_building_effect(city, "cavalry_siege_xp_pct")
        } else {
            0.0
        };
        if spec.domain.as_deref() == Some("air") {
            xp_pct += self.city_building_effect(city, "air_unit_xp_pct");
        }
        if spec.domain.as_deref() == Some("sea") && spec.class == "military" {
            xp_pct += self.city_building_effect(city, "naval_unit_xp_pct");
        }
        if spec.domain.as_deref().is_none() && spec.class == "military" {
            xp_pct += self.city_building_effect(city, "all_land_unit_xp_pct");
        }
        let starting_xp = self.city_building_effect(city, "unit_starting_xp") as i64;
        let cavalry_movement = if spec.cavalry {
            self.city_building_effect(city, "cavalry_movement")
        } else {
            0.0
        };
        let unit = self.units.get_mut(&uid).unwrap();
    ...
    /// Begin a military conquest. Ownership changes immediately so the melee
    /// unit can enter, but victory and elimination are deferred until the
    ...
    fn pending_city_capture(&self, pid: usize, cid: u32) -> Result<usize, String> {
        let city = self
            .cities
            .get(&cid)
            .ok_or_else(|| "no such captured city".to_string())?;
        if city.owner != pid {
            return Err("not your captured city".into());
        }
        city.captured_from
    ...
    fn capture_rewards(&mut self, conqueror: usize, defeated: usize, grievances: f64) {
        ...
        let duration = self
            .cities
            .values()
            .filter(|city| city.owner == conqueror)
            .map(|city| self.city_building_effect(city, "capture_city_duration"))
            .fold(0.0, f64::max) as i64;
        if duration > 0 {
            self.players[conqueror].counters.insert(
                "warlords_throne_until".to_string(),
                self.turn as i64 + duration,
            );
        }
    }

    fn city_state_elimination_grievances(&mut self, defeated: usize, conqueror: usize) {
    ...
    fn do_keep_city(&mut self, pid: usize, cid: u32) -> Result<(), String> {
        ...
        self.city_state_elimination_grievances(defeated, pid);
        self.check_domination();
        Ok(())
    }
    ...
    fn do_raze_city(&mut self, pid: usize, cid: u32) -> Result<(), String> {
        ...
        if city.is_capital {
            return Err("capitals cannot be razed".into());
        }
...
mod victory_conditions {
    ...
    fn occupied_cities_create_recurring_grievances_and_capitals_drain_favor() {
        ...
        assert_eq!(g.players[1].grievances.get(&0), Some(&51.0));
        assert!((g.players[0].diplomatic_favor - 5.49).abs() < 1e-9);
    }
    ...
    fn eliminating_a_city_state_angers_other_majors_and_liberation_restores_it() {
        ...
        assert_eq!(g.players[1].grievances.get(&0), Some(&100.0));
...
mod district_mechanics {
    ...
    fn district_great_person_points_include_unique_rates_and_lavra_building_gates() {
        ...
        game.process_great_people(0);
        assert_eq!(game.players[0].gpp.get("prophet"), Some(&6.0));

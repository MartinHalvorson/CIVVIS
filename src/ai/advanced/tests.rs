...
fn timed_war_prebuild_floor_counts_units_and_queues() {
    ...
    assert!(
        filled < 7_000.0,
        "the exact four-body vacancy disappears once filled"
    );
}
...
fn timed_war_holds_an_incomplete_package_then_declares_together() {
    ...
    assert_eq!(
        staged.len(),
        TIMED_WAR_BODIES,
        "fixture needs four lawful staging tiles"
    );
...
fn a_wartime_naval_recon_arm_reserves_one_idle_coastal_city() {
    ...
    for distance in 1..=super::super::NAVAL_RECON_MIN_WATERWAY_TILES {
        current = game
            ...
            .into_iter()
            .find(|pos| {
                game.map.tiles.contains_key(pos) && game.wdist(city_pos, *pos) == distance as i32
            })
    ...
    assert!(live.live_war_economy_requires_recovery(&bankrupt, 0, &live.counts(&bankrupt, 0)));
    ...
    let interiors: Vec<Pos> = game
        ...
        .filter(|tile| {
            game.wdisk(tile.pos, 4).len() == 61
                && city_positions
                    .iter()
                    .all(|city| game.wdist(*city, tile.pos) >= 5)
        })
        .map(|tile| tile.pos)
        .collect();
...
fn advanced_settlers_wait_out_a_visible_hostile_capture_envelope() {
    ...
    assert!(
        ai.settlement_tile_risk(&game, 0, None, civilian_step, &visible) > SETTLER_STEP_RISK_LIMIT
    );
    game.at_war.clear();
    assert_eq!(
...
fn a_warship_off_the_coast_threatens_a_settler_on_the_shore_and_nowhere_inland() {
    ...
    let mut lands: Vec<Pos> = game
        .map
        .tiles
        .keys()
        .copied()
        .filter(|p| is_land(&game, *p))
        .collect();
    lands.sort_unstable();
    let (shore, sea) = lands
        .iter()
...
fn a_threatened_settler_on_the_shore_retreats_inland_but_holds_inside_a_city() {
    ...
    let (seed, shore, sea, inland) =
        found.expect("some seed offers a shore with dry ground behind it");
    ...
    for _ in 0..SETTLER_RETREAT_LIMIT {
        ...
        if fresh_thoughts
            .iter()
            .any(|t| t.headline.starts_with("Settler retreats from"))
        {
            retreats += 1;
        }
    }
    assert_eq!(
        retreats, SETTLER_RETREAT_LIMIT,
        "each turn on the shore is a retreat"
    );
    assert!(
...
fn conquest_ai_spends_a_device_on_the_hard_city_but_spares_its_own() {
    ...
    assert_eq!(
        game.players[0].counters["project_effect:thermonuclear_devices"], 1,
...
fn strategic_settler_routes_to_an_island_beyond_the_local_search_radius() {
    ...
    assert!(
        ...
        "the target stays on the useful island"
    );
...
fn fleet_objective_treats_embarked_enemies_as_naval_contacts() {
    ...
    let _observer = g.spawn_test_unit("galley", 0, anchor);
    let embarked = g.spawn_test_unit("settler", 1, contact);
    assert!(
        g.player_can_see(0, contact),
        "the fleet can only target an embarked contact it can actually see"
    );
    let plan = StrategicPlan {
        strategy: GrandStrategy::Conquest,
        target_player: Some(1),
...
fn outmatched_major_must_negotiate_peace_with_the_winning_campaign() {
    ...
    assert!(game
        .pending_deals
        .iter()
        .any(|deal| { deal.from == 1 && deal.to == 0 && deal.peace }));
...
fn strategic_planning_excludes_unmet_zero_power_majors() {
    ...
    assert_eq!(
        expansion_sites.len(),
        2,
        "fixture needs two vacant city sites"
    );
...
fn coupled_expansion_charges_the_paid_sequence_and_closes_without_payback() {
    ...
    let early = treated.coupled_expansion_value(&game, 0, city, &plan, &counts, site, build_turns);
    assert!(
        early > -10_000.0,
        "a healthy early site should remain an auditable paid candidate: {early}"
    );
    let forecast =
        treated.settlement_growth_forecast_from_positions(&game, 0, site.0, &game.wdisk(site.0, 2));
...
fn live_campaign_holds_a_major_war_city_until_capture_or_emergency() {
    ...
    let ranked_target = ranked
        .target_city
        .expect("the major war has a city objective");
...
fn campaign_city_ordering_prefers_a_breach_then_the_domination_capital() {
    ...
    let outpost_position = game
        ...
        .min_by_key(|(position, _)| {
            (
                (game.wdist(game.cities[&own_capital].pos, **position) - capital_distance).abs(),
                **position,
            )
        })
        .map(|(position, _)| *position)
    ...
    let _capital_observer = game.spawn_test_unit("scout", 0, game.cities[&enemy_capital].pos);
    let _outpost_observer = game.spawn_test_unit("scout", 0, game.cities[&enemy_outpost].pos);
    ...
    let _capital_refresh = game.spawn_test_unit("scout", 0, game.cities[&enemy_capital].pos);
    let _outpost_refresh = game.spawn_test_unit("scout", 0, game.cities[&enemy_outpost].pos);
    ai.belief.observe(&game, 0);
    assert!(
...
fn conquest_army_stages_before_diplomacy_opens_the_war() {
    ...
    let staging: Vec<Pos> = game
        ...
        .into_iter()
        .filter(|position| {
            (3..=5).contains(&game.wdist(*position, objective)) && game.city_at(*position).is_none()
        })
        .take(army.len())
        .collect();
...
fn recovery_requires_material_local_danger_and_ends_when_it_clears() {
    ...
    let intruder_pos = game
        .wdisk(home_pos, 6)
        .into_iter()
        .find(|position| game.wdist(*position, home_pos) == 3 && game.city_at(*position).is_none())
        .unwrap();
    ...
    let far_pos = game
        ...
        .keys()
        .copied()
        .find(|position| game.wdist(*position, home_pos) >= 9 && game.city_at(*position).is_none())
        .unwrap();
...
fn a_defensible_site_score_discounts_camps_and_isolation() {
    ...
    plain.settlement_safety = false;
    let mut weighed = AdvancedAi::new();
    weighed.settlement_safety = false;
    weighed.defensible_sites = true;
    assert!(!plain.defensible_sites, "the measurement flag ships off");
...
#[test]
fn settlement_site_score_prioritizes_growth_water_and_future_districts() {
    let game = Game::new_full(2, 30, 18, 8_114, 120, 0, false);
    let center =
        game.map
            .tiles
            .keys()
            .copied()
            .find(|position| {
                game.wdisk(*position, 3)
                    .into_iter()
                    .all(|near| game.map.get(near).is_some())
                    && game.map.get(*position).is_some_and(|tile| {
                        !game.rules.is_water(tile) && game.rules.is_passable(tile)
                    })
            })
            .expect("the test map has an interior land tile");
    let food_tile = game
        .nbrs(center)
        .into_iter()
        .find(|position| game.map.get(*position).is_some())
        .expect("the center has a neighbor");
    let mut food = game.clone();
    let mut production = game.clone();
    for candidate in [&mut food, &mut production] {
        for position in candidate.wdisk(center, 1) {
            if let Some(tile) = candidate.map.tiles.get_mut(&position) {
                tile.terrain = Name::new("desert");
                tile.feature = None;
                tile.resource = None;
    ...
    food.map.tiles.get_mut(&food_tile).unwrap().terrain = Name::new("grassland");
    production.map.tiles.get_mut(&food_tile).unwrap().terrain = Name::new("plains");
    let ai = AdvancedAi::new();
    assert!(
        ai.settle_value(&food, 0, center) > ai.settle_value(&production, 0, center) + 1.0,
        "early food should beat the same tile's production-only alternative"
    );

    let mut freshwater = food.clone();
    assert!(freshwater.map.set_river_edge(center, food_tile, true));
    assert!(
        ai.settle_value(&freshwater, 0, center) > ai.settle_value(&food, 0, center) + 10.0,
        "fresh water should be a material founding advantage"
    );

    let mut district = game.clone();
    for position in district.wdisk(center, 3) {
        if let Some(tile) = district.map.tiles.get_mut(&position) {
    ...
    let district_site = district
        .nbrs(center)
        .into_iter()
        .find(|position| {
            district.map.get(*position).is_some_and(|tile| {
                !district.rules.is_water(tile) && district.rules.is_passable(tile)
            })
        })
        .expect("the normalized ring has a district site");
    let mountain = district
        .nbrs(district_site)
        .into_iter()
        .find(|position| {
            *position != center
                && district.wdist(center, *position) <= 3
...
fn settlement_atlas_parallel_misses_match_uncached_values_and_invalidate_on_map_write() {
    ...
    assert_eq!(
        cloned, expected,
        "a worker game clone must preserve static site values"
    );
    ...
    let cloned_controller = positions
        ...
        .collect::<Vec<_>>();
    assert_eq!(
        cloned_controller, expected,
        "a worker controller clone must preserve static site values"
    );
    ...
    assert_eq!(
        actual, expected,
        "parallel atlas scoring must be bit-identical"
    );
    ...
    assert_eq!(
        refreshed, uncached,
        "a map epoch change must refresh the atlas"
    );
}
...
fn settlement_forecast_prefers_four_sustainable_jobs_to_one_spike() {
    ...
    assert!(
        spike.reached_population < 4,
        "one tile should stall: {spike:?}"
    );
    assert_eq!(
        broad.reached_population, 4,
        "four jobs should mature: {broad:?}"
    );
    assert!(
...
#[test]
fn advanced_settler_refuses_a_visible_direct_attack_tile() {
    let mut game = Game::new_full(2, 30, 18, 8_115, 120, 0, false);
    let initial = game
        .player_unit_ids(0)
        .into_iter()
        .find(|unit| game.units[unit].kind == "settler")
        .expect("player zero starts with a settler");
    let home = game.units[&initial].pos;
    let origin =
        game.map
            .tiles
            .keys()
            ...
            .find(|position| {
                game.wdist(home, *position) > 8
                    ...
                    && game
                        .units
                        .values()
                        .all(|unit| unit.owner != 0 || game.wdist(unit.pos, *position) > 2)
            })
    ...
    let threat =
        game.nbrs(next)
            ...
            .expect("the route has a visible attack square");
    let enemy = game.spawn_test_unit("warrior", 1, threat);
    game.at_war.insert((0, 1));
    let ai = AdvancedAi::new();
    let visible = ai.battlefront_visibility(&game, 0);
    let planned_risk = ai.settlement_tile_risk(&game, 0, Some(settler), next, &visible);
    assert!(game.sees(&visible, threat));
    assert!(
        planned_risk > SETTLER_STEP_RISK_LIMIT,
        "the visible warrior should make the planned tile obviously unsafe: \
             risk={planned_risk}, distance={}, at_war={}",
        game.wdist(threat, next),
        game.is_at_war(0, 1)
    );
    let moving = ai;
    let _ = moving.settler_step_toward_safe(&mut game, 0, settler, target);
    assert_ne!(
        game.units[&settler].pos, next,
        "a settler must not step into a visible direct attack envelope"
    );
    assert!(game.units.contains_key(&enemy));
}

#[test]
fn settlement_safety_is_shared_by_top_advanced_constructors() {
    assert!(AdvancedAi::new().settlement_safety);
    assert!(AdvancedAi::targeting(VictoryTarget::Culture).settlement_safety);
    assert!(AdvancedAi::with_weights(Weights::default()).settlement_safety);
    assert!(!AdvancedAi::legacy().settlement_safety);
}
...
fn bulk_rival_culture_pressure_matches_individual_victory_scans() {
    ...
    for pid in 0..4 {
        let batched = ai.rival_victory_pressure_with_culture(&game, pid, bulk.get(&pid).copied());
...
fn competitive_religious_opening_produces_multiple_founders() {
    let mut game = Game::new_full(
        ...
        24,
        16,
        crate::rng::fixture_seed("PROPHET", 76_106),
        110,
        0,
        false,
    );
...
fn the_live_seat_races_the_score_leader_instead_of_declaring_on_it() {
    ...
    assert_eq!(
        pressure.strategy,
        GrandStrategy::Expansion,
        "the score clock names the leader"
    );
    assert!(
        pressure.progress >= 78,
        "and calls it close to winning ({})",
        pressure.progress
    );
    assert_eq!(
    ...
    assert_eq!(
        withheld.victory_denial(&game, 0),
        Some((2, GrandStrategy::Conquest))
    );
...
fn the_empire_reserves_one_launch_pad_in_the_city_that_would_run_the_race() {
    ...
    assert!(
        passed_over > 0.0,
        "a second launch site stays a legal fallback"
    );
...
fn envoy_priority_reaches_adaptive_production_and_obeys_safety_gates() {
    ...
    assert!(!treatment.prioritize_envoy_infrastructure(&mut recovery, 0, &recovery_plan));
    ...
    assert!(!treatment.prioritize_envoy_infrastructure(&mut threatened, 0, &threatened_plan));

    let mut rushing = game.clone();
    rushing.record_contact(0, minor);
    let mut rush_plan = plan.clone();
    rush_plan.rush = true;
    assert!(!treatment.prioritize_envoy_infrastructure(&mut rushing, 0, &rush_plan));
    ...
    major_war
        ...
        .unwrap();
    assert!(!treatment.prioritize_envoy_infrastructure(&mut major_war, 0, &plan));
    ...
    occupied
        ...
        .unwrap();
    assert!(!treatment.prioritize_envoy_infrastructure(&mut occupied, 0, &plan));
    ...
    assert!(treatment.prioritize_envoy_infrastructure(&mut consulate, 0, &plan));
    assert!(matches!(
    ...
    assert!(treatment.prioritize_envoy_infrastructure(&mut chancery, 0, &plan));
    assert!(matches!(
    ...
    assert!(!treatment.prioritize_envoy_infrastructure(&mut expired, 0, &plan));
...
fn a_theater_square_owes_its_buildings_the_way_a_campus_does() {
    ...
    let capped = live.district_project_value(&game, 0, capital, "commercial_hub_investment", &plan);
    let free =
    ...
    // The constants are known at compile time, so clippy folds the
    // comparison to a literal; the ordering is still the thing under
    // test, so assert on values it cannot fold.
    assert!(
        std::hint::black_box(CULTURE_BUILDING_DEBT) < std::hint::black_box(RESEARCH_BUILDING_DEBT)
    );
...
fn a_regional_amenity_building_counts_the_cities_it_reaches() {
    ...
    assert!(
        game.can_produce(0, capital, &zoo),
        "the fixture must offer the Zoo"
    );
    let spec = game.rules.buildings["zoo"].clone();
    assert_eq!(
        spec.regional_range, range,
        "the Zoo is a six-tile regional building"
    );
    ...
    assert!(
        reached >= 1,
        "the fixture sites a city inside the Zoo's range"
    );
...
fn force_readiness_excludes_aircraft_from_ground_armies() {
    ...
    assert!(ai.force_groups.iter().all(|group| {
        group
            .units
            .iter()
            .all(|unit| game.rules.units[game.units[unit].kind].domain.as_deref() != Some("air"))
    }));
}
    ...
    assert!(ai.force_groups.iter().all(|group| {
        group.units.iter().all(|left| {
            group
                .units
                .iter()
                .all(|right| game.wdist(game.units[left].pos, game.units[right].pos) <= 6)
        })
    }));
    assert!(!ai
        .force_groups
...
fn local_superiority_prices_the_objective_city_defense() {
    ...
    let ratio = ai.local_strength_ratio(&game, 0, &[warrior], &[1], game.cities[&target_city].pos);
...
fn bomber_exact_result_search_prefers_a_kill_over_static_strength() {
    ...
    let mut targets: Vec<Pos> =
        game.wdisk(base, game.rules.units["bomber"].range)
            .into_iter()
            .filter(|position| {
                *position != base
                    && game.map.get(*position).is_some_and(|tile| {
...
fn bomber_planners_choose_high_value_air_pillage_over_low_value_strikes() {
    ...
    let base =
        game.wdisk(target, game.rules.units["bomber"].range)
            .into_iter()
            .find(|position| {
                *position != target
...
fn jet_planners_priority_target_escorted_air_defenses() {
    ...
    let base = game
        ...
        .unwrap();
    let target =
        game.wdisk(base, game.rules.units["jet_bomber"].range)
            .into_iter()
            .find(|position| {
                *position != base
                    && game.map.get(*position).is_some_and(|tile| {
    ...
    assert_eq!(
        AdvancedAi::targeting(VictoryTarget::Domination).advanced_air_action(&game, 0, jet, &plan),
        Some(expected)
    );
}
...
fn army_declines_a_captured_settler_when_no_city_site_remains() {
    ...
    let target = game
        ...
        .unwrap();
    let city_centers: BTreeSet<Pos> = game.cities.values().map(|city| city.pos).collect();
    for (position, tile) in &mut game.map.tiles {
        if ![home, origin, target].contains(&position) && !city_centers.contains(&position) {
            tile.terrain = crate::name!("ocean");
            tile.feature = None;
        }
    }
    ...
    let capture_result = direct_capture.apply(
        0,
        &Action::Move {
            unit: warrior,
            to: target,
    ...
    assert!(
        ...
        "staged capture was not legal: {capture_result:?}"
    );
...
fn conquest_waits_to_recapture_an_unholdable_original_capital() {
    ...
    let pressure_city = game.found_city_for(1, pressure_site, Some("Pressure".to_string()));
    ...
    let mut ai = AdvancedAi::targeting(VictoryTarget::Domination);
    let _target_observer = game.spawn_test_unit("scout", 0, game.cities[&target_city].pos);
    let _pressure_observer = game.spawn_test_unit("scout", 0, game.cities[&pressure_city].pos);
    ai.belief.observe(&game, 0);
    ...
    assert!(ai
        .tactical_attack_value(&supported, 0, attacker, &capture, &plan)
        .is_finite());
...
fn project_search_maintains_aged_reactors_and_marginal_conversion_avoids_churn() {
    ...
    let original_stock_value = 450.0
    ...
    game.cities.get_mut(&city).unwrap().buildings.extend([
        ...
        crate::name!("nuclear_power_plant"),
    ]);
    assert!(
        marginal.production_value(&game, 0, city, &coal, &plan, &counts) < -9_000.0,
...
fn defensive_recovery_force_gap_outranks_rear_city_great_person_project() {
    ...
    let award =
        game.project_completion_gpp_awards(0, rear_city, "theater_square_festival")["writer"];
...
fn live_gold_purchase_spends_through_the_reserve_to_save_a_besieged_city() {
    ...
    let action = live
        ...
        .find(|action| match action {
            Action::Buy {
                ...
                formation,
                currency,
            } => *action_city == city && unit == &wanted && *formation == 0 && currency == "gold",
            _ => false,
        })
    ...
    assert!(
        cost > 0.0 && cost < 300.0,
        "fixture must sit below reserve: {cost}"
    );
    ...
    assert_eq!(
        purchased.len(),
        1,
        "the emergency spends one action, not a Builder"
    );
    ...
    assert_eq!(
        unit.pos, city_pos,
        "the defender appears in the city it saves"
    );
    assert!(
    ...
    assert_eq!(
        game.players[0].gold, 0.0,
        "saving the city outranks the reserve"
    );
}
...
fn strategic_gold_purchase_annexes_a_luxury_and_preserves_the_reserve() {
    ...
    assert!(
        AdvancedAi::targeting(VictoryTarget::Science).advanced_gold_spending(&mut game, 0, &plan)
    );
...
fn deep_treasury_buys_useful_infrastructure_in_multiple_cities() {
    ...
    let stock_purchases = game
        ...
        .collect::<Vec<_>>();
    assert_eq!(ai.legal_purchase_actions(&game, 0), stock_purchases);
    let mut parallel_ai = ai.clone();
    parallel_ai.work_pool = Some(Arc::new(WorkPool::new(4)));
    assert_eq!(
        parallel_ai.legal_purchase_actions(&game, 0),
        stock_purchases
    );
    let mut parallel_game = game.clone();

    assert!(ai.advanced_gold_spending(&mut game, 0, &plan));
    assert!(parallel_ai.advanced_gold_spending(&mut parallel_game, 0, &plan));
    assert_eq!(
...
fn live_wartime_bankruptcy_slots_the_available_maintenance_discount() {
    ...
    assert!(!build(false, 0.0, false).contains(&crate::name!("conscription")));
    assert!(!build(true, 100.0, false).contains(&crate::name!("conscription")));
...
fn explicit_targets_replace_early_cards_with_victory_policies() {
    ...
    assert!(culture.players[0]
        .policies
        .contains(&crate::name!("heritage_tourism")));
    assert!(culture.players[0]
        .policies
...
fn future_policy_reassessment_replaces_ancient_fillers() {
    ...
    assert!(game.players[0]
        .policies
        .contains(&crate::name!("five_year_plan")));
    assert!(game.players[0]
        .policies
        .contains(&crate::name!("rationalism")));
    assert!(ancient
...
fn policy_reassessment_only_executes_swaps_that_can_fit() {
    ...
    assert!(valid.players[0]
        .policies
        .contains(&crate::name!("logistics")));
    assert!(!valid.players[0]
        .policies
        .contains(&crate::name!("rationalism")));
    assert_eq!(
...
fn dark_age_policies_follow_strategy_without_retired_cards() {
    ...
    assert!(!game.players[0]
        .policies
        .contains(&crate::name!("isolationism")));
...
fn advanced_trader_uses_an_unreserved_destination_empire_wide() {
    ...
    assert!(AdvancedAi::new().advanced_trader_step(&mut game, 0, trader, GrandStrategy::Expansion,));
...
fn strategic_governments_use_late_tiers_and_match_the_culture_holdout() {
    ...
    AdvancedAi::new().strategic_government(&mut science_fallback, 0, GrandStrategy::Science);
    assert_eq!(
    ...
    AdvancedAi::new().strategic_government(&mut religion_fallback, 0, GrandStrategy::Religion);
    assert_eq!(
...
fn adaptive_government_does_not_repeat_lateral_anarchy() {
    ...
    assert_eq!(
        game.players[0].pending_government.as_deref(),
        Some("fascism")
    );
...
fn culture_builders_upgrade_farms_to_resorts_without_reverting_them() {
    ...
    assert_eq!(
        upgrades.first().map(|name| name.as_str()),
        Some("seaside_resort")
    );
...
fn live_envoy_bank_invests_the_pool_above_its_liquidity_reserve() {
    ...
    assert_eq!(
        game.players[0].envoys_free, BANKED_ENVOY_LIQUIDITY_RESERVE,
...
fn coordinated_mover_ignores_a_hostile_missing_from_its_turn_start_frame() {
    let choose = |ai: &AdvancedAi, game: &mut Game, unit: u32, orders: &ForceGroup| {
        ai.coordinated_tactical_step(game, 0, unit, orders, &[1], false)
            .then_some(())?;
        match game.log.last() {
            Some((actor, Action::Move { unit: moved, to })) if *actor == 0 && *moved == unit => {
                Some(*to)
            }
            _ => None,
        }
...
fn friendly_volley_reprices_a_two_unit_kill_after_the_finisher() {
    ...
    assert_eq!(
        firing_lines.len(),
        2,
        "interior tile needs two firing lines"
    );
    ...
    let game = (2..=100)
        .find_map(|hp| {
            ...
            (after_first.apply(0, &opening).is_ok() && after_first.units.contains_key(&defender))
    ...
    assert!(
        played.units.contains_key(&defender),
        "the opening shot is deliberately nonlethal"
    );
    assert!(matches!(
        played.log.last(),
...
fn forcing_reply_search_avoids_a_poisoned_capture() {
    ...
    assert!(parallel_ai.advanced_military_step(&mut parallel_game, 0, attacker, &plan));
    assert_eq!(
...
fn parallel_planner_quality_census() {
    ...
    for pair in 0..pairs {
        for swap in 0..2 {
            ...
            let candidate = |pid: usize| (pid + swap).is_multiple_of(2);
...
fn colliding_unit_intents_revalidate_and_finish_from_live_state() {
    ...
    let (target, origins) = game
        ...
        .find_map(|target| {
            if game
                .map
                .get(target)
                .is_none_or(|tile| !game.rules.is_passable(tile) || game.rules.is_water(tile))
            {
                return None;
            }
            let origins = game
                ...
                .take(2)
                .collect::<Vec<_>>();
            (origins.len() == 2).then_some((target, origins))
        })
        .expect("test map has two approaches to one land tile");
    let first = game.spawn_test_unit("warrior", 0, origins[0]);
    let second = game.spawn_test_unit("warrior", 0, origins[1]);
    game.current = 0;
    ...
    let intents = [first, second]
        .into_iter()
        .map(|unit| UnitIntent {
            actions: vec![Action::Move { unit, to: target }],
            took_a_turn: true,
            base_state: ai.base.unit_plan_state(unit),
        })
        .collect();
...
fn basil_stages_hippodromes_then_resumes_them_at_divine_right() {
    let mut game = Game::new_full(
        ...
        24,
        16,
        crate::rng::fixture_seed("BASIL", 111),
        200,
        0,
        false,
    );
...
#[test]
fn campaign_city_score_uses_the_last_seen_city_combat_state() {
    let mut game = Game::new_full(2, 30, 18, 411_007, 120, 0, false);
    let settler = game
        .player_unit_ids(1)
        .into_iter()
        .find(|unit| game.units[unit].kind == "settler")
        .expect("second player has a settler");
    game.current = 1;
    game.apply(1, &Action::FoundCity { unit: settler })
        .expect("foreign city can be founded");
    ...
    let observer = game.spawn_test_unit("scout", 0, city_pos);
    let mut ai = AdvancedAi::new();
    ai.belief.observe(&game, 0);
    assert!(
        ai.remembered_city(city).is_some(),
        "the City Center was seen"
    );
    ...
    game.spawn_test_unit("giant_death_robot", 1, city_pos);
    let after = ai.campaign_city_value(&game, 0, &game.cities[&city], GrandStrategy::Conquest);
...
fn a_camp_within_nine_tiles_of_a_city_is_home_ground_the_guard_clears() {
    ...
    assert_eq!(
        withheld
            .base
            .home_defense_objective(&game, 0, warrior, &[barb]),
        None,
    ...
    assert!(!BasicAi::barbarian_presence_at_home_with_camp_radius(
        &game,
        0,
...
fn a_relief_column_aims_at_the_siege_not_at_its_own_doorstep() {
    ...
    assert!(
        game.wdist(city_pos, loiterer_pos) <= 8,
        "both are candidates"
    );
    ...
    let nearest_to_us = ai.domain_objective(&game, 0, &plan, ForceDomain::Land, anchor, &enemies);
    ai.enable_relief_targets_the_siege();
    let nearest_to_city = ai.domain_objective(&game, 0, &plan, ForceDomain::Land, anchor, &enemies);
...
fn a_force_is_judged_at_the_radius_it_was_assembled_at() {
    ...
    let home = *land
        ...
        .max_by_key(|pos| {
            (
                land.iter()
                    .filter(|other| game.wdist(**pos, **other) <= 6)
                    .count(),
                std::cmp::Reverse(**pos),
            )
        })
...
fn belief_pressure_remembers_a_hidden_contact_without_reading_current_state() {
    ...
    let hidden =
        game.wdisk(city_pos, THREAT_RELIEF_RADIUS)
            .into_iter()
            .find(|position| {
                *position != city_pos
                    && game.city_at(*position).is_none()
    ...
    let scout = scout_candidates
        ...
        .find_map(|position| {
            let scout = game.spawn_test_unit("scout", 0, position);
            let sees_enemy =
                game.player_visibility(0).contains(&hidden) && game.unit_visible_to(enemy, 0);
            if sees_enemy {
                Some(scout)
            } else {
                game.remove_unit(scout);
                ...
                *totals.entry(directive.role.as_str()).or_default() += 1;
                ...
                let friendly = game.city_strength(*cid)
                    + game
                        ...
                        .filter(|unit| game.rules.units[unit.kind].class == "military")
                        .map(|unit| {
                            crate::game::effective_strength(game.unit_strength(unit, true), unit.hp)
                        })
                        .sum::<f64>();
...
#[test]
fn city_pressure_matrix_matches_serial_unit_order_exactly() {
    let mut game = Game::new_full(2, 30, 18, 91_210, 80, 0, false);
    let first = found_test_city(&mut game, 0);
    let second = found_test_city(&mut game, 0);
    let enemy = found_test_city(&mut game, 1);
    game.at_war.insert((0, 1));
    let positions = [
        game.cities[&first].pos,
        game.cities[&second].pos,
        game.cities[&enemy].pos,
    ];
    for index in 0..64 {
...
fn city_loss_autopsy() {
    ...
    for map in 0..30u64 {
        ...
        // (pop, food, starving, production, gold, science, districts).
        type CitySnapshot = (i32, i32, bool, i32, i32, i32, usize);
        let mut snap: std::collections::HashMap<u32, CitySnapshot> =
        ...
        while game.winner.is_none() && game.turn <= game.max_turns {
            ...
            let now: std::collections::HashSet<u32> = game.player_city_ids(0).into_iter().collect();
            for gone in owned.difference(&now) {
                ...
                match game.cities.get(gone).map(|c| c.owner) {
                    None => razed += 1,
                    Some(other) if game.players.get(other).is_some_and(|p| p.is_free_city) => {
                        revolted += 1
                    }
                ...
                let Some(&(hp, maxhp, garrison, near3, near6, adj, army)) = snap.get(gone) else {
                    continue;
                };
                lost += 1;
    ...
    let garrison_calls = crate::ai::GARRISON_STEP_CALLS.load(std::sync::atomic::Ordering::Relaxed);
    println!("garrison_step reached {garrison_calls} times across the whole census");
    let pct = |n: u64| 100.0 * n as f64 / lost.max(1) as f64;
    println!(
        "cities lost: {lost}  (mean army alive at the moment of loss: {:.1})",
        our_army_total as f64 / lost.max(1) as f64
    );
    println!(
        "  besieged (hp < max) ....... {besieged:>3}  {:>5.1}%",
        pct(besieged)
    );
    println!(
        "  lost at FULL hp ........... {full_hp:>3}  {:>5.1}%  <- not a siege",
        pct(full_hp)
    );
    println!(
        "  an enemy stood ADJACENT ... {enemy_adjacent:>3}  {:>5.1}%",
        pct(enemy_adjacent)
    );
    println!(
        "  had a GARRISON ............ {garrisoned:>3}  {:>5.1}%",
        pct(garrisoned)
    );
    println!(
        "  our military within 3 ..... {help_within_3:>3}  {:>5.1}%",
        pct(help_within_3)
    );
    println!(
        "  our military within 6 ..... {help_within_6:>3}  {:>5.1}%",
        pct(help_within_6)
    );
    println!(
        "  NO military within 6 ...... {no_help_at_all:>3}  {:>5.1}%",
        pct(no_help_at_all)
    );
    ...
    for (d, n) in enemy_dist_hist.iter().enumerate() {
        if *n > 0 {
            ...
            println!(
                "    distance {label:<3} .................. {n:>3}  {:>5.1}%",
                pct(*n)
            );
        }
    }
    println!(
        "  BEYOND GARRISON_ALERT_RADIUS ({}) .. {beyond_alert:>3}  {:>5.1}%",
        crate::ai::GARRISON_ALERT_RADIUS,
        pct(beyond_alert)
    );
    println!(
        "  an enemy could MOVE+STRIKE it that turn {could_strike:>3}  {:>5.1}%",
        pct(could_strike)
    );
    println!("WHO HOLDS IT NOW (ground truth the live export cannot give):");
    println!(
        "  REVOLTED to Free Cities (loyalty 0) ... {revolted:>3}  {:>5.1}%",
        pct(revolted)
    );
    println!(
        "  taken by an enemy we are at war with .. {taken_by_enemy:>3}  {:>5.1}%",
        pct(taken_by_enemy)
    );
    println!(
        "  taken by a city-state/minor ........... {taken_by_minor:>3}  {:>5.1}%",
        pct(taken_by_minor)
    );
    println!(
        "  owner changed with NO war (revolt) .... {flipped_peacefully:>3}  {:>5.1}%",
        pct(flipped_peacefully)
    );
    println!(
        "  razed / gone .......................... {razed:>3}  {:>5.1}%",
        pct(razed)
    );
}
...
fn come_ashore_census() {
    for &flag in &[false, true] {
        ...
        for map in 0..30u64 {
            ...
            let mut owned: std::collections::HashSet<u32> =
                game.player_city_ids(0).into_iter().collect();
            let mut prev_hp: std::collections::HashMap<u32, i32> = std::collections::HashMap::new();
            while game.winner.is_none() && game.turn <= game.max_turns {
                let pid = game.current;
                ais[pid].take_turn(&mut game, pid);
                if game.winner.is_none() && game.current == pid {
    ...
    for map in 0..8u64 {
        ...
        while game.winner.is_none() && game.turn <= game.max_turns {
            ...
            for cid in game.player_city_ids(0) {
                ...
                if city.buildings.iter().any(|b| b.contains("walls")) && walled.insert((map, cid)) {
                    walls_built += 1;
...
fn settle_siting_census() {
    ...
    let disc_mean = discounted_ratios.iter().sum::<f64>() / discounted_ratios.len().max(1) as f64;
    println!(
...
fn settle_search_reach_census() {
        ...
        while game.winner.is_none() && game.turn <= game.max_turns {
            ...
            if pid == 0 {
                ...
                for uid in game.player_unit_ids(0) {
                    ...
                    settler_turns += 1;
                    let returned = BasicAi::first_reachable_settle_site(&game, uid, &candidates);
    ...
    println!("  candidates the generator offered above what the search returned: {better_offered}");
    println!(
...
fn settle_value_prices_district_adjacency_geometry() {
    ...
    for pos in &ring2 {
        if scattered.len() < 3
            && scattered
                .iter()
                .all(|picked| game.wdist(*picked, *pos) >= 3)
        {
            scattered.push(*pos);
        }
    }
    assert_eq!(
        scattered.len(),
        3,
        "ring two admits three mutually distant peaks"
    );
    ...
    assert_eq!(
        cluster.len(),
        3,
        "a ring-one plot touches three ring-two plots"
    );
    for pos in &cluster {
        game.map.tiles.get_mut(pos).unwrap().terrain = crate::name!("mountain");
    }
...
fn settle_value_before_and_after_founding() {
    ...
    println!(
        "\n=== settle_value before and after founding: {} sites ===",
        before_after.len()
    );
...
fn expansion_funnel_at(label: &str, players: usize, width: i32, height: i32) {
    ...
    for map in 0..8u64 {
        let mut game = Game::new_full(players, width, height, 480_000 + map, 200, 1, false);
        let mut ais: Vec<AdvancedAi> = (0..game.players.len()).map(|_| AdvancedAi::new()).collect();
        game.set_fog_memory(false);
        while game.winner.is_none() && game.turn <= game.max_turns {
            let pid = game.current;
    ...
    println!(
        "  a settler already walking (serialization)  {fail_serial:>6}  ({:>5.1}%)",
        pct(fail_serial)
    );
    println!(
        "  city below pop 2                           {fail_pop:>6}  ({:>5.1}%)",
        pct(fail_pop)
    );
    println!(
        "  expansion window shut                      {fail_window:>6}  ({:>5.1}%)",
        pct(fail_window)
    );
    println!(
        "  no settle site in reach                    {fail_site:>6}  ({:>5.1}%)",
        pct(fail_site)
    );
    println!(
        "  NOTHING blocking — the settler was allowed {all_clear:>6}  ({:>5.1}%)",
        pct(all_clear)
    );
...
fn expansion_payback_fires() {
    for (label, players, width, height) in [
        ...
        println!(
            "  payback OPENS what the flat reserve shut   {opened:>6}  ({:>5.1}%)",
            opened as f64 / total.max(1) as f64 * 100.0
        );
        println!(
            "  payback SHUTS what the flat reserve opened {closed:>6}  ({:>5.1}%)",
            closed as f64 / total.max(1) as f64 * 100.0
        );
    }
    println!();
}
...
fn production_allocation_census() {
    for (label, players, width, height) in [
        ...
        for (kind, amount) in rows {
            println!(
                "  {kind:<14} {amount:>10.0}  ({:>5.1}%)",
                amount / total.max(1.0) * 100.0
            );
        }
        println!(
...
fn plan_city_target_fires() {
    for (label, players, width, height) in [
        ...
        for flag in [false, true] {
            ...
            let maps = 6u64;
            for map in 0..maps {
                let mut game = Game::new_full(players, width, height, 482_000 + map, 200, 1, false);
                let mut ais: Vec<AdvancedAi> = (0..game.players.len())
                    .map(|_| {
                        let mut ai = AdvancedAi::new();
                        ai.plan_city_target = flag;
...
fn idle_treasury_census() {
    for (label, players, width, height) in [
        ...
        println!(
            "\n=== idle treasury [{label}]: {} seat-turns ===",
            gold_samples.len()
        );
        println!(
...
fn faith_spending_census() {
    ...
    for (shape, players, width, height, city_states, turns, speed, victories) in [
        ...
        for map in 0..maps {
            ...
            game.victory_conditions = crate::game::VictoryConditions::parse(victories).unwrap();
            let mut ais: Vec<AdvancedAi> =
                (0..game.players.len()).map(|_| deployed_agent()).collect();
            game.set_fog_memory(false);
            ...
            if game.player_city_ids(0).iter().any(|cid| {
...
fn envoy_allocation_census() {
    ...
    for (arm, priced) in [("stock", false), ("suzerainty priced", true)] {
        for (shape, players, width, height, city_states) in [
            ...
            println!(
                "\n=== envoy allocation [{label}]: {} seat-turns ===",
                free_samples.len()
            );
            println!(
            ...
            println!(
                "  envoys placed on the board     mean {:.1}",
                mean(&placed_samples)
            );
            println!(
...
fn envoy_income_census() {
    ...
    for (label, players, width, height, city_states) in [
        ...
        for map in 0..maps {
            ...
            while game.winner.is_none() && game.turn <= game.max_turns {
                ...
                *govs.entry(gov.to_string()).or_default() += 1;
                ...
                if game.cities.values().any(|c| {
                    c.owner == 0
                        && c.districts
                            .keys()
                            .any(|d| d.as_str() == "diplomatic_quarter")
        ...
        for (name, count) in rows.into_iter().take(4) {
            println!(
                "    {name:<24} {:>5.1}% of turns",
                *count as f64 / turns.max(1) as f64 * 100.0
            );
        }
        ...
        for card in CARDS {
            ...
            println!(
                "    {card:<22} {:>5.1}% of turns",
                n as f64 / turns.max(1) as f64 * 100.0
            );
        }
        ...
        println!(
            ...
            civic_turns.get("ideology").copied().unwrap_or(0) as f64 / turns.max(1) as f64 * 100.0
        );
        println!(
            "    a diplomatic slot open     {:>5.1}% of turns",
            diplo_slot_turns as f64 / turns.max(1) as f64 * 100.0
        );
        println!("    diplomatic_quarter built   in {diplo_quarter_games} of {maps} games");
...
fn a_rising_stock_pressure_reads_urgent_a_congress_earlier() {
    ...
    let mut live = AdvancedAi::new();
    live.enable_live_bridge();
    assert!(live.projected_stock_denial, "the live seat carries the treatment");
    assert!(live.stock_denial_lead_time, "the projection rides the lead-time bar");
    ...
    live.stock_pressure_history
        .insert(1, (0..=8).map(|i| (100 + i, 41 + (i as i32) * 12 / 5)).collect());
    assert!(live.victory_pressure_is_urgent(&game, 1, rising));
    ...
    withheld.stock_pressure_history
        .insert(1, (0..=8).map(|i| (100 + i, 41 + (i as i32) * 12 / 5)).collect());
    ...
    disable(&mut via_row);
    assert!(!via_row.projected_stock_denial, "{field} row disables the flag");

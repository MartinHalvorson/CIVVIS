...
impl AdvancedAi {
    ...
    pub(crate) fn coalition_target(
        ...
        if !self.coalition_before_war && !self.coalition_before_war_3 {
            return None;
        }
        let appointed = self
            .war_plan
    ...
    ///
    /// This neither infers an Advanced recipient's hidden plan from the
    /// Basic controller's acceptance rule nor spends a ready strike waiting
    /// for a Joint War. The bilateral grievance and denunciation
...
mod tests {
    ...
    fn v3_allies_with_an_active_front_without_holding_the_declaration() {
        ...
        // military proposal, spends no Envoys and holds no declaration.
        ai.coalition_alliance_step(&mut game, 0);
        let proposal = game
            .pending_deals
            .iter()

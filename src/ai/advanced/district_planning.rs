...
impl AdvancedAi {
    ...
    pub(super) fn district_plan_plot_score(
        ...
        // ask an idle city to start now. A purchased plot is useful only once
        // its district can enter the next production decision, and this
        // preserves the full ordinary reserve until then.
        if self.district_planning_3 && (row_index != 0 || !g.cities[&city].queue.is_empty()) {
            return None;
        }
        let row = &planned[row_index];
        let variant = row.district;
        let (min_adjacency, min_edge) = (PLAN_BUY_MIN_ADJACENCY, PLAN_BUY_MIN_EDGE);
        let here = g

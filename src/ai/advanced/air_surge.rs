...
/// the path to [`AIR_SURGE_GOAL_TECH`] — the operator's "three tech levels
/// out". From an empire holding Industrialization, Scientific Theory and
/// Steam Power that is exactly `flight`, `radio`, `advanced_flight`; anything
/// deeper is a research plan, not a surge, and the empire has better things to
/// do with forty turns of beakers.
pub(crate) const AIR_SURGE_TECH_HORIZON: usize = 3;
...
impl AdvancedAi {
    ...
    pub(crate) fn maintain_air_surge(&mut self, g: &Game, pid: usize) {
        ...
        let missing_now = Self::air_surge_missing_techs(g, pid);
        self.air_surge_closest = self.air_surge_closest.min(missing_now);
        let research_turns = (Self::war_remaining_research_cost(
            g,
            pid,
            Name::new(AIR_SURGE_GOAL_TECH),
        ) / Self::war_science_per_turn(g, pid))
        .ceil() as u32;
        self.air_surge_reach
            .entry(missing_now)
            .or_insert((g.turn, research_turns));

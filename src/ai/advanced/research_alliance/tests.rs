...
/// The whole-game instrument: plays the screen's own size (6 majors, 74×46,
/// 250 turns) with the gene on every major and reports, per major, the turn
/// Civil Service and Scientific Theory landed, the alliances held at the
/// end, and the desk's counter. Not an assertion — whether a Research
/// Alliance is reachable on a 250-turn clock is the question it measures.
///
/// ```text
...
fn research_alliance_whole_game_instrument() {
    for seed in [26_081_900_u64, 26_081_901, 26_081_902] {
        let mut world = Game::new_full(6, 74, 46, seed, 250, 9, true);
        ...
        for pid in 0..6 {
            ...
            let alliances: Vec<String> = p
                ...
                .collect();
            println!(
                "seed {seed} pid {pid} {} civil_service={:?} scientific_theory={:?} alliances={:?} asked={:?} score={}",
                p.civ,
                civil_service[pid],

...
impl AdvancedAi {
    ...
    fn victory_focus(&self, g: &Game, pid: usize) -> VictoryFocus {
        ...
        let science = project_progress.max(travel_progress).max(readiness);
        ...
        if g.players[pid].civic.is_none() {
            ...
            .or_else(|| {
                (plan.strategy == GrandStrategy::Science
                    && !g.players[pid].civics.contains("space_race"))
                .then_some("space_race")
            });
            let goal_pick = forced_goal.and_then(|goal| {
                available
                    .iter()
    ...
    fn religious_spending(&self, g: &mut Game, pid: usize, offensive: bool) {
        ...
        let priorities = if home_under_pressure && inquisition_launched && inquisitors < 2 {
            vec!["inquisitor", "apostle", "missionary", "guru"]
        } else if !offensive {
            vec!["missionary", "inquisitor"]
        } else if apostles < 2 {
...
mod tests {
    ...
    fn expansion_window_reaches_its_six_city_target_before_endgame() {
        let mut game = Game::new_full(1, 30, 18, 7_111, 500, 0, false);
        let settler = game
            .player_unit_ids(0)
            .into_iter()
            .find(|unit| game.units[unit].kind == "settler")

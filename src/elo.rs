...
use crate::game::{Action, Game, GameOptions, TournamentPreset};
use crate::rng::Rng;
    ...
    C: FnMut(&[RatedPlayer]) -> Result<(), E>,
{
    ...
    for gi in 0..cfg.games {
        let gseed = cfg.seed.wrapping_mul(100_000).wrapping_add(gi as u64);
        let seats = scheduled_seats(
            names,
        ...
        if cfg.verbose {
            ...
            let labels: Vec<String> = results
                ...
                .collect();
            println!(
                "game {gi:3}  winner={wname:<10} ({}, {}, t{})  seats={labels:?}",
...
mod tests {
    use super::{
        dominant_strategy, scheduled_seats, seat_schedule, EloPool, RatedPlayer, RatingKey,
        ELO_SCHEMA_VERSION,
    };
    use crate::rng::Rng;

...
mod tests {
    ...
    fn thermopylae_is_a_pass_and_not_a_field() {
        ...
        assert!(north >= width / 2, "the Malian Gulf should hold the north edge");
        assert!(south >= width / 2, "Kallidromos should hold the south edge");
    }
    ...
    /// French frontage and the mud drowned their advance.
    #[test]
    fn agincourt_is_a_corridor_between_two_woods() {
        let map = chart("agincourt");
        let woods = |north: bool| {
            map.tiles.values().filter(|tile| {
                let (_, row) = hex::axial_to_offset(tile.pos.0, tile.pos.1);
                tile.feature.as_deref() == Some("forest")
                    && if north { row < map.height / 2 } else { row >= map.height / 2 }
            })
        };
        assert!(woods(true).count() >= 6, "the Tramecourt wood is missing");
        assert!(woods(false).count() >= 6, "the Azincourt wood is missing");
        assert!(
            tiles_where(&map, |tile| tile.feature.as_deref() == Some("marsh")) >= 4,
    ...
    /// Roman flank and shortened the frontage they could deploy on.
    #[test]
    fn cannae_is_pinned_against_the_aufidus() {
        ...
        let river = map
            .tiles
            .values()
            .filter(|tile| tile.river_edges.iter().any(|edge| *edge))
            .count();
        assert!(river >= 8, "the Aufidus is missing from the field");
    }
    ...
    fn naval_and_landing_charts_hold_the_water_they_claim() {
        for scenario in crate::historical_scenarios::generic_scenarios() {
            ...
            match scenario.terrain {
                ...
                _ => assert!(
                    water * 8 <= map.tiles.len(),
                    "{} is a land battle on {water} water tiles",
                    scenario.id
                ),
            }
        }

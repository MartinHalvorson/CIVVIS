...
fn civ6_unit_type(name: &civvis::name::Name) -> String {
    let id = match name.as_str() {
        ...
        "samurai" => "JAPANESE_SAMURAI",
        "hypaspist" => "MACEDONIAN_HYPASPIST",
        "varu" => "INDIAN_VARU",
        "mandekalu_cavalry" => "MALI_MANDEKALU_CAVALRY",
        "cossack" => "RUSSIAN_COSSACK",
...
#[cfg(test)]
use civvis::ai::finishing::finish_live_war_units_excluding;
#[cfg(test)]
use civvis::ai::finishing::{finish_live_war_units, live_finishing_candidates};
...
fn decide(
    ...
    let decision = serde_json::json!({
        ...
        "native_actions": planned_game.log.since(before)
            ...
            .map(|(_,action)| action).collect::<Vec<_>>(),
        "finishing_lines": war_finishers.execution,
        "unit_ids": mirror_state.civ6_of,
        "city_ids": mirror_state.cid_of,
        "unmapped": mirror_state.unmapped,
    });
    format!(
        "{{\"turn\":{},\"orders\":[{}],\"note\":{},\"decision\":{}}}",
        state.turn,
        body,
    ...
    fn delegations_and_embassies_name_the_firaxis_seat() {
        ...
        for (casus_belli, statement) in [
            ...
            let war = translate(
                &Action::DeclareWarWithCasusBelli {
                    player: 1,
                    casus_belli: casus_belli.into(),
                },
                &mirror,
                &state,
            )
            .expect("a supported declaration retains its native session");
            assert_eq!(war.subject, Some(4));
            assert_eq!(war.verb.as_deref(), Some(statement));
        }
        for unsupported in ["joint_war", "unknown_war"] {
            assert!(translate(
                &Action::DeclareWarWithCasusBelli {
                    player: 1,
                    casus_belli: unsupported.into(),
                },
                &mirror,
                &state,
            )
...
mod order_postcondition_tests {
    ...
    fn diplomacy_is_verified_by_the_answer_and_the_war_flag() {
        ...
        let war = order("war", Some(2), Some("DECLARE_FORMAL_WAR"), None);
    ...
    fn host_war_permission_reopening_clears_only_the_resolved_declaration_refusal() {
        let war = order("war", Some(3), Some("DECLARE_FORMAL_WAR"), None);

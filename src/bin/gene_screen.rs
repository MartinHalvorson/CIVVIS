...
fn on_probabilities(genes: &[Gene], screened: &[bool], p_on: f64, p_default_on: f64) -> Vec<f64> {
    genes
        .iter()
        .zip(screened)
        .map(|(gene, &is_screened)| match (is_screened, gene.default_on) {
...
fn draw_genome(start_seed: u64, game: usize, players: usize, seat: usize, probabilities: &[f64]) -> Vec<bool> {
    let mut rng = Rng::new(
        start_seed
...
fn play_game(
    ...
    majors
        .iter()
        .enumerate()
        .map(|(index, &seat)| row_for_seat(&world, game, seed, seat, genome_string(&genomes[index]), secs))
        .collect()
}

/// One finished game read from one seat's point of view.
fn row_for_seat(game: &Game, index: usize, seed: u64, seat: usize, genome: String, secs: f64) -> Row {
...
impl<'a> Seats<'a> {
    fn of(header: &Header, rows: &'a [Row]) -> Self {
        ...
        let wins = rows.iter().map(|row| f64::from(u8::from(row.win))).collect();
    ...
    fn contrast(&self, column: usize, outcome: &[f64]) -> (f64, f64) {
        let design: Vec<Vec<f64>> = self.signs.iter().map(|row| vec![1.0, row[column]]).collect();
    ...
    fn adjusted(&self, outcome: &[f64]) -> Option<Vec<(f64, f64)>> {
        ...
        Some(fit.into_iter().skip(1).map(|(b, se)| (2.0 * b, 2.0 * se)).collect())
    }
}
...
fn estimate(header: &Header, rows: &[Row]) -> Estimates {
    ...
    let overall_win = if n > 0 { seats.wins.iter().sum::<f64>() / n as f64 } else { 0.0 };
    let overall_share = if n > 0 { seats.shares.iter().sum::<f64>() / n as f64 } else { 0.0 };
...
fn interactions(header: &Header, rows: &[Row], outcome: fn(&Row) -> f64) -> (Vec<Interaction>, usize) {
    let seats = Seats::of(header, rows);
    let n = seats.rows.len();
    if n < 3 {
        return (Vec::new(), n);
    ...
    let centres: Vec<f64> = seats.columns.iter().map(|&i| 2.0 * probabilities[i] - 1.0).collect();
...
fn print_by_civ(header: &Header, rows: &[Row], tag: &str) {
    ...
    let mut table: Vec<(&str, usize, f64, f64, f64, f64)> = by_civ
        ...
        .map(|(civ, seats)| {
            ...
            let wins: Vec<f64> = seats.iter().map(|row| f64::from(u8::from(row.win))).collect();
        ...
        .collect();
    table.sort_by(|a, b| {
        let za = if a.3 > 0.0 && a.3.is_finite() { a.2 / a.3 } else { 0.0 };
        let zb = if b.3 > 0.0 && b.3.is_finite() { b.2 / b.3 } else { 0.0 };
        zb.total_cmp(&za)
    });
...
fn main() {
    ...
    for retired in ["--pairs", "--target-pairs", "--anchor-pairs", "--design", "--single-seat", "--field", "--baseline", "--p-helps", "--p-hurts", "--p-unresolved"] {
...
mod tests {
    ...
    fn synthetic(k: usize, games: usize, outcome: impl Fn(&[bool], usize) -> (bool, f64)) -> (Header, Vec<Row>) {
    ...
    fn ols_recovers_planted_effects() {
        ...
        let seats = Seats::of(&header, &rows);
        let fitted = seats.adjusted(&seats.shares).expect("1,200 seats support 5 genes");
        for (i, (effect, se)) in fitted.iter().enumerate() {
            ...
            assert!(se.abs() < 1e-6, "noise-free fit should carry no error, got {se}");
        }
    ...
    fn estimate_reads_a_planted_gene_and_clears_an_inert_one() {
        ...
        assert!((planted.win_on - 2.0 / 3.0).abs() < 0.06, "on {}", planted.win_on);
        assert!(planted.win_off.abs() < 1e-9);
        assert!((planted.win_delta - 2.0 / 3.0).abs() < 0.06, "Δ {}", planted.win_delta);
        assert!(planted.win_z() > 10.0, "z {}", planted.win_z());
        assert!(planted.n_on > 350 && planted.n_off > 350, "{}/{}", planted.n_on, planted.n_off);
        let inert = &estimates.genes[1];
        assert!(inert.win_z().abs() < 3.0, "inert gene z {}", inert.win_z());
        assert!(inert.win_delta.abs() < 0.12, "inert gene Δ {}", inert.win_delta);
    ...
    fn errors_cluster_by_game() {
        let design: Vec<Vec<f64>> = (0..40).map(|i| vec![1.0, if i % 2 == 0 { 1.0 } else { -1.0 }]).collect();
        let y: Vec<f64> = (0..40).map(|i| if i % 2 == 0 { 1.0 } else { 0.0 } + (i % 5) as f64 * 0.1).collect();
        ...
        assert!(se_naive < 0.8 * se_clustered, "naive {se_naive} vs clustered {se_clustered}");
    }
    ...
    fn reproducibility_tranches_are_newest_first_and_whole_game() {
        ...
        assert!(tranches.iter().all(|t| t.seats % 3 == 0), "{:?}", tranches.iter().map(|t| t.seats).collect::<Vec<_>>());
    ...
    fn interactions_recover_a_planted_synergy() {
        let (header, rows) = synthetic(3, 1500, |bits, _| {
            ...
            (false, 0.5 + 0.1 * s(bits[0]) + 0.05 * s(bits[1]) * s(bits[2]))
        });
        ...
        assert!((synergy.synergy - 0.2).abs() < 0.03, "4γ should be near 0.2, got {}", synergy.synergy);
        assert!(synergy.z() > 10.0, "z {}", synergy.z());
        let none = pair(0, 1);
        assert!(none.synergy.abs() < 0.03, "a main effect is not a synergy: {}", none.synergy);
    }
    ...
    fn timing_cost_is_per_enabled_seat() {
        ...
        assert!((g0.compute_pct() - 10.0).abs() < 0.05, "{}", g0.compute_pct());
    ...
    fn rows_without_timings_publish_no_cost() {
        ...
        assert!(estimate_costs(&header, &rows).is_empty(), "secs are 0.0 in test rows");
    }
    ...
    fn rows_round_trip_through_json_and_a_legacy_row_still_reads() {
        ...
        assert!(!text.contains("\"pair\""), "a new row carries no pair: {text}");
        assert!(!text.contains("\"arm\""), "a new row carries no arm: {text}");
